...
const ExpandedControls = `
	<div class="main-nowPlayingView-lyricsControls">
		<button id="Romanize" class="Psc33HXPyazZYAAr1tgz"></button>
		<button id="Page" class="Psc33HXPyazZYAAr1tgz VpNHGG5ZhoxQ8AqW709S">
...
export default class CardView implements Giveable {
	...
	constructor(
		...
		// Handle our button-presses
		{
			...
			if (this.Lyrics.RomanizedLanguage !== undefined) {
				const romanizedLanguage = this.Lyrics.RomanizedLanguage
	...
	private CreateLyricsRenderer() {
		this.Maid.Give(
			new LyricsRenderer(
				this.LyricsRootContainer, this.Song, this.Lyrics,
				(
					(this.Lyrics.RomanizedLanguage === undefined)
					? false
